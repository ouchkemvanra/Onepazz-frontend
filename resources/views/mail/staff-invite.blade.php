...
<p>You have been added as <strong>{{ ucfirst($staffRecord->role) }}</strong> at <strong>{{ $staffRecord->gym->name }}</strong> on the KhmerFit platform.</p>
@if($tempPassword)
...
<p><a href="{{ url('/login') }}" style="background:#0d9488;color:white;padding:10px 20px;border-radius:6px;text-decoration:none;display:inline-block;">Login to KhmerFit</a></p>
@else
<p>You can now access the check-in screen at <a href="{{ url('/gym-portal/checkin-screen') }}">{{ url('/gym-portal/checkin-screen') }}</a>.</p>
@endif
<p style="color:#6b7280;font-size:12px;margin-top:24px;">KhmerFit — Cambodia's Fitness Network</p>
</body>
</html>
