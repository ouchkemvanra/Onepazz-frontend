...
<head>
    ...
    <title>Payments — Admin — KhmerFit</title>
...
<div class="max-w-7xl mx-auto px-6 py-8">
    ...
    <div class="mb-8">
        ...
        <p class="text-sm text-gray-400 mt-1">Review and confirm employer payment submissions</p>
    </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50">
                <tr class="border-b border-gray-200">
                    <th class="text-left text-xs font-semibold text-gray-400 uppercase tracking-wide px-6 py-4">Payment ID</th>
                    <th class="text-left text-xs font-semibold text-gray-400 uppercase tracking-wide px-6 py-4">Employer</th>
                    <th class="text-left text-xs font-semibold text-gray-400 uppercase tracking-wide px-6 py-4">Amount</th>
                    <th class="text-left text-xs font-semibold text-gray-400 uppercase tracking-wide px-6 py-4">Transfer Date</th>
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-50 hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4">
                        <div class="font-medium text-gray-800">{{ $payment->employer->company_name }}</div>
                        <div class="text-xs text-gray-400">Invoice #{{ $payment->invoice->invoice_number }}</div>
                    </td>
                    <td class="px-6 py-4">
                        <div class="font-semibold text-gray-800">${{ number_format($payment->amount_usd, 2) }}</div>
                        <div class="text-xs text-gray-400">{{ number_format($payment->amount_khr) }} ៛</div>
                    </td>
                    <td class="px-6 py-4 text-gray-600">{{ $payment->transfer_date?->format('d M Y') }}</td>
                    <td class="px-6 py-4">
                        @if($payment->status === 'pending')
                            <span class="bg-orange-50 text-orange-600 border border-orange-200 text-xs font-medium px-2 py-1 rounded-full">Pending Review</span>
                    ...
                    <td class="px-6 py-4">
                        <a href="{{ route('admin.payments.show', $payment) }}" class="text-teal-600 hover:text-teal-700 font-medium">Review →</a>
                    </td>
                </tr>
                @empty
                <tr><td colspan="6" class="py-12 text-center text-gray-400 text-sm">No payments found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
